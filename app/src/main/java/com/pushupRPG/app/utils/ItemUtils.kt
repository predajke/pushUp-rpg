...
object ItemUtils {
    ...
    fun getSetBonuses(equippedItems: List<com.pushupRPG.app.data.model.Item>): SetBonuses {
        ...
        for ((setId, count) in setCounts) {
            when (setId) {
                ...
                "hellxdead" -> { dmg += if (count >= 3) 1.50f else if (count >= 2) 0.80f else 0f; drop += if (count >= 3) 0.50f else 0f }
                "void"      -> {
                    dmg  += when {
                        count >= 5 -> 2.50f
                        count >= 4 -> 1.50f
    ...
    private data class ItemsWrapper(val items: List<com.pushupRPG.app.data.model.Item>)
}
